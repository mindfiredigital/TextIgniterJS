...
export class InsertLayoutHandler {
  ...
  private activeLayout: HTMLDivElement | null = null;
  ...
  private insertLayout(columns: number, widths: number[]) {
    ...
    setTimeout(() => {
      const firstCell = wrapper.querySelector('.layout_column') as HTMLElement;
      if (firstCell) {
        firstCell.focus();
        this.setActiveLayout(
          wrapper.querySelector('.layout_container') as HTMLDivElement
        );
      }
    }, 0);
  }
  ...
  private setupCellEvents(cell: HTMLElement) {
    cell.addEventListener('focus', () => {
      const layout = cell.closest('.layout_container') as HTMLDivElement;
    ...
    cell.addEventListener('keydown', (e: Event) => {
      ...
      if (ke.key === 'Tab') {
        ...
        const allCells = Array.from(
          cell
            .closest('.layout_container')
            ?.querySelectorAll('.layout_column') || []
        ) as HTMLElement[];
  ...
  private setActiveLayout(layout: HTMLDivElement) {
    document.querySelectorAll('.layout_active').forEach(t => {
      if (t !== layout) t.classList.remove('layout_active');
    });
