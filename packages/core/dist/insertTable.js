...
export class InsertTableHandler {
    ...
    insertTable(rows, cols) {
        ...
        deleteBtn.onclick = () => {
            const index = this.document.blocks.findIndex((b) => b.dataId === tableId);
            if (index !== -1) {
        ...
        this.document.emit('documentChanged', this.document);
        setTimeout(() => {
            const firstCell = wrapper.querySelector('.tblCell');
            if (firstCell) {
                firstCell.focus();
                this.setActiveTable(wrapper.querySelector('table'));
            }
        }, 0);
    }
    setupCellEvents(cell) {
        ...
        cell.addEventListener('keydown', (e) => {
            ...
            if (ke.key === 'Tab') {
                ke.preventDefault();
                ke.stopPropagation();
                const allCells = Array.from(((_a = cell.closest('table')) === null || _a === void 0 ? void 0 : _a.querySelectorAll('.tblCell')) || []);
