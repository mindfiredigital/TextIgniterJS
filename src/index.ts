...
import {
  showHyperlinkViewButton,
  hideHyperlinkViewButton,
} from "./attributes/hyperLink";
...
class TextIgniter {
  ...
  createEditor(editorId: string, config: EditorConfig) {
    ...
    config.features.forEach((feature) => {
      if (feature === "fontFamily") {
        ...
        featuresWithPngIcon.map((item) => item.feature).indexOf(feature) !== -1
      ) {
        const featureDataArray = featuresWithPngIcon.filter(
  ...
  handleToolbarAction(action: string, dataId: string[] = []): void {
    ...
    switch (action) {
      case "orderedList":
        this.document.dataIds.map((obj: string, i: number) =>
          this.document.toggleOrderedList(obj, i + 1)
        );
        // this.document.toggleOrderedList(this.document.selectedBlockId)

        break;
      case "unorderedList":
        this.document.dataIds.map((obj) =>
          this.document.toggleUnorderedList(obj)
        );
      ...
      switch (action) {
        ...
        case "hyperlink":
          if (start < end) {
            ...
            const existingLink = this.document.getCommonHyperlinkInRange(
              start,
              end
            );
  ...
  handleSelectionChange(): void {
    ...
    if (parentBlock && parentBlock instanceof HTMLElement) {
      this.document.selectedBlockId =
        parentBlock.getAttribute("data-id") || null;
    }
  }
  ...
  handleKeydown(e: KeyboardEvent): void {
    ...
    if (e.key === "Enter") {
      ...
      if (
        this.document.blocks[this.document.blocks.length - 1]?.listType ===
          "ol" ||
        this.document.blocks[this.document.blocks.length - 1]?.listType === "ul"
      ) {
        const ListType =
          this.document.blocks[this.document.blocks.length - 1]?.listType;
        let _start = 1;
        if (ListType === "ol") {
          _start =
            this.document.blocks[this.document.blocks.length - 1]?.listStart;
          _start += 1;
        }
          ...
          if (extractedContent.length > 0) {
            updatedBlock = this.addBlockAfter(
              ...
              this.getCurrentCursorBlock()!.toString(),
              {
                dataId: uniqueId,
                class: "paragraph-block",
                pieces: [new Piece(extractedContent)],
  ...
  addBlockAfter(data: any[], targetDataId: string, newBlock: any): any[] {
    ...
    const targetIndex = data.findIndex(
      (block) => block.dataId === targetDataId
    );
